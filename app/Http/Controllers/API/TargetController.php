...
class TargetController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $workers = $this->resolveWorkers($authUser, $requestedUserId, $month);
    ...
    private function resolveWorkers(User $authUser, ?int $requestedUserId, ?string $month = null): Collection
    {
        if (!$this->canManageTargets($authUser)) {
        ...
        if ($requestedUserId) {
            ...
            $query->whereExists(function ($exists) use ($monthStart, $monthEnd) {
                $exists
                    ->selectRaw('1')
                    ->from('task_user')
                    ->join('tasks', 'tasks.id', '=', 'task_user.task_id')
                    ->whereColumn('task_user.user_id', 'users.id')
                    ->where(function ($taskQuery) use ($monthStart, $monthEnd) {
                        $taskQuery
                            ->whereBetween('tasks.start_at', [$monthStart, $monthEnd])
                            ->orWhereBetween('tasks.end_at', [$monthStart, $monthEnd])
                            ->orWhereBetween('tasks.created_at', [$monthStart, $monthEnd])
                            ->orWhereBetween('tasks.updated_at', [$monthStart, $monthEnd]);
                    });
            });
        }
    ...
    private function ensureWorker(User $user, string $field): void
    {
        ...
        throw ValidationException::withMessages([
            $field => 'Only active users can receive monthly targets.',
        ]);
    }
    ...
    private function canAccessTarget(?User $authUser, Target $target): bool
    {
        ...
        return $this->canManageTargets($authUser)
            || ((int) $target->user_id === (int) $authUser->id);
    }
